...
export function generateItineraryPDF(
  ...
  if (itinerary.flights && itinerary.flights.length > 0) {
    ...
    itinerary.flights.forEach((flight, index) => {
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.text(`${flight.departure || 'N/A'} → ${flight.arrival || 'N/A'}`, 18, yPosition + 5);
  ...
  if (itinerary.hotels && itinerary.hotels.length > 0) {
    ...
    doc.text('🏨 Accommodation', 15, yPosition);
    yPosition += 8;
    ...
    itinerary.hotels.forEach((hotel, index) => {
      ...
      doc.setFontSize(9);
      doc.setFont('helvetica', 'normal');
      doc.text(`${hotel.location || 'Location TBD'} • ${hotel.nights} night${hotel.nights > 1 ? 's' : ''}`, 18, yPosition + 5);
  ...
  if (itinerary.dailyPlan && itinerary.dailyPlan.length > 0) {
    ...
    doc.text('📅 Daily Itinerary', 15, yPosition);
    yPosition += 10;
